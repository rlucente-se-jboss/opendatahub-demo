...
PUSHD ${WORKDIR}
    ...
    echo "    oc port-forward ceph-nano-0 8080 &"
    echo
    ...
    echo "    aws --endpoint-url http://127.0.0.1:8080 s3 mb s3://\${S3_BUCKET_NAME}"
    echo "    aws --endpoint-url http://127.0.0.1:8080 s3 cp \${DATA_FILENAME} s3://\${S3_BUCKET_NAME}"
    echo "    aws --endpoint-url http://127.0.0.1:8080 s3 ls s3://\${S3_BUCKET_NAME}"
    echo
POPD
